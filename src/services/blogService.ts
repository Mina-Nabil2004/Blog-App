...
export async function getBlog(id: string): Promise<Blog> {
    ...
    if (!blog) throw ApiError.notFound({ id: `Blog with id ${id} not found` });
    return blog;
}
...
export async function publishBlog(id: string, authorID: string): Promise<Blog> {
    const blog = await prisma.blog.findUnique({ where: { blogID: id } });
    if (!blog) throw ApiError.notFound({ id: `Blog with id ${id} not found` });
    if (blog.authorID !== authorID) throw ApiError.forbidden({ blog: "You are not the author of this blog" });
    if (blog.published) throw ApiError.badRequest({ blog: "Blog is already published" });
...
export async function unpublishBlog(id: string, authorID: string): Promise<Blog> {
    ...
    if (!blog) throw ApiError.notFound({ id: `Blog with id ${id} not found` });
    if (blog.authorID !== authorID) throw ApiError.forbidden({ blog: "You are not the author of this blog" });
    if (!blog.published) throw ApiError.badRequest({ blog: "Blog is already unpublished" });
